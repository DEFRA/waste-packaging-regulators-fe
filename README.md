...
all locally — visit the certificates-of-compliance pages after `npm run dev`. The
mock is stateless: accepting or cancelling a certificate returns the same fixture
data on the next request (the success banner is driven by the session, not the mock).
This keeps every request deterministic and makes parallel test runs safe.
