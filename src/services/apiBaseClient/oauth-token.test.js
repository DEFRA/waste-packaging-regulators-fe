...
describe('getServiceOAuthAccessToken', () => {
  test('requests a client_credentials token and returns the access token', async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(tokenResponse({ access_token: 'tok-abc', expires_in: 3600 }))

    const token = await getServiceOAuthAccessToken({ ...baseOptions, fetchImpl })
  ...
  test('caches the token across calls and only fetches once', async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(tokenResponse({ access_token: 'tok-cached', expires_in: 3600 }))

    const first = await getServiceOAuthAccessToken({ ...baseOptions, fetchImpl })
    const second = await getServiceOAuthAccessToken({ ...baseOptions, fetchImpl })
  ...
  test('coalesces concurrent refreshes into a single token request', async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(tokenResponse({ access_token: 'tok-single', expires_in: 3600 }))
  ...
  test('throws when the token response has no access_token', async () => {
    const fetchImpl = vi.fn().mockResolvedValue(tokenResponse({ expires_in: 3600 }))
